...
const _createClusterScript = (info, filePath) => {
    ...
    env.CODEFRESH_URL = `${context.url}/`;
    fs.chmodSync(filePath, '755');
    const clusterScript = spawn(filePath, ['create', '--c', name, '--token', context.token], { env });
...
const createCluster = async (info) => {
    const dirPath = `${homedir()}/Codefresh`;
    const filePath = `${homedir()}/Codefresh/stevedore`;
    const versionPath = `${homedir()}/Codefresh/version.txt`;
    const versionUrl = 'https://raw.githubusercontent.com/codefresh-io/Stevedore/master/VERSION';
    let zipPath = `${homedir()}/Codefresh/data`;
    ...
    if (shouldUpdate) {
        ...
        req.on('end', () => {
            ...
            //check if working with windows
            //var zip = new AdmZip('/Users/codefresh/Codefresh/stevedore_1.1.2_Windows_x86_64.zip');
            decompress(zipPath, `${homedir()}/Codefresh`, {
                plugins: [
                    decompressTargz(),
                ],
            }).then(() => {
