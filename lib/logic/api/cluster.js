...
const _createClusterScript = (info, filePath) => {
    const { name, context, namespace, serviceaccount } = info;
    fs.chmodSync(filePath, '755');
    const clusterScript = spawn(filePath, ['create', '--c', name, '--token', context.token, '--api-host', `${context.url}/`, '--namesapce', namespace, '--serviceaccount', serviceaccount] );
