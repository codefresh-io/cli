...
// context classes
const { JWTContext , APIContext } = require('./contexts');
...
class Manager {
    ...
    loadContexts(configFilePath, cfToken, cfUrl) {
        ...
        try {
            ...
            _.forEach(doc.contexts, (rawContext) => {
                switch (rawContext.type) {
                    ...
                    case APIContext.TYPE: {
                        const context = APIContext.createFromSerialized(rawContext);
                        this.addContext(context);
                        break;
                    }
                    default: {
        ...
        try {
            ...
            try {
                if (cfToken && cfUrl) {
                    const context = APIContext.createFromToken(cfToken, cfUrl);
                    this.addContext(context);
                    this.setCurrentContext(context);
                }
            } catch (err) {
