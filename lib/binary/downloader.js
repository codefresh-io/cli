...
async function _getRemoteVersion({
    ...
    for (let i = 0; i < releases.length; i += 1) {
        ...
        const neededAsset = (curRelease.assets || []).find(asset => _.get(asset, 'name', '').endsWith(osType));
        if (neededAsset) {
            return { name: curRelease.name, download_url: neededAsset.browser_download_url };
        }
    }
    return { name: '0', download_url: '' };
}
...
class Downloader {
    ...
    async download(component) {
        ...
        let { name: remoteVersion, download_url } = await _getRemoteVersion({ name, osType });
