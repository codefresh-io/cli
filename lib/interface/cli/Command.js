...
class Command {
    constructor(command) {
        ...
        delete command.root;
        this.needAuthentication = command.needAuthentication || true;
        ...
        let funcs = _.map(this.builders, (b) => {
            return (yargs) => {
                ...
                _.forEach(this.subCommands, (subCommand) => {
                    ...
                    if (subCommand instanceof Command && (!subCommand.isBetaCommand() || (!_.isEmpty(authManager.getAllContexts()) && authManager.getCurrentContext().isBetaFeatEnabled()))) {
