...
const command = new Command({
    ...
    builder: (yargs) => {
        return yargs
            ...
            .option('source-control', {
                ...
                type: Array,
                choices: ['github', 'bitbucket' , 'stash' , 'gitlab'],
            })
    ...
    handler: async (argv) => {
        ...
        const sourceControl = argv['source-control'];
        const pipelineName = argv['pipeline-name'];

        let pipelines;
        // TODO:need to decide for one way for error handeling
        if (id && repoOwner && repoName) {
            pipelines = await pipeline.getPipelineByNameAndRepo(id, repoOwner, repoName, sourceControl);
        } else if (id) {
            pipelines = await pipeline.getPipelineById(id);
        } else if (repoOwner && repoName) {
            pipelines = await pipeline.getAllByRepo(repoOwner, repoName, sourceControl, pipelineName);
        } else {
            pipelines = await pipeline.getAll(sourceControl, pipelineName);
        }
