...
const { prepareKeyValueFromCLIEnvOption, prepareEncryptedValues } = require('../../helpers/general');
...
class RunBaseCommand {
    ...
    async run() {
        ...
        if (variablesFromFile) {
            ...
            const variables = prepareEncryptedValues(this.argv.variable, this.argv.encrypted);
            // const variables = prepareKeyValueFromCLIEnvOption(this.argv.variable);
            // const variables = this.argv.variable;
            const request = _.cloneDeep(executionRequestTemplate);
            request.options.variables = variables;
    ...
    async preRunAll() {
        ...
        // const variableMap = _.reduce(this.argv.variable, (acc, v) => _.assign(acc, { [v.key]: v }), {});
        // _.forEach(this.argv.encrypted, (varName) => {
