...
class RunBaseCommand {
    ...
    async run() {
        ...
        if (this.isParalel) {
            await Promise.all(this.executionRequests.map(request => this.runImpl(request)));
        } else {
            const chainedPromise = this.executionRequests.reduce((promisedChain, request) => promisedChain.then((chainResult) => {
                this.runImpl(request).then(currentResult => [...chainResult, currentResult]);
            }), Promise.resolve([]));
