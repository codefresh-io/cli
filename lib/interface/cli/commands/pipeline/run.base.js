...
const { prepareKeyValueFromCLIEnvOption, prepareEncryptedValues, prepareKeyValueObjectsFromEnvFileOption } = require('../../helpers/general');
...
class RunBaseCommand {
    ...
    async run() {
        ...
        if (variablesFromFile) {
            requestVariables = prepareKeyValueObjectsFromEnvFileOption(variablesFromFile);
        } else {
            requestVariables = this.argv.variable;
        }

        const variables = prepareEncryptedValues(requestVariables, this.argv.encrypted);
