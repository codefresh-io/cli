...
class RunExternalCommand extends RunBaseCommand {
    async runImpl(request) {
        ...
        this.workflowId = await sdk.pipelines.run({ name: pipelineName }, { requestBody: _buildBody(options) });
        if (this.executionRequests.length === 1) {
            ...
            const buildResponse = await sdk.builds.get({ buildId: this.workflowId });
