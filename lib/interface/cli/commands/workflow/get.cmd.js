        ...
        if (workflowId) {
            ...
            if (_.isEmpty(pipelineIds) && !_.isEmpty(pipelineNames)) {
                ...
                _.forEach(pipelineNames, (pipelineName) => {
                    ...
                    if (matchPipelines) {
                        const arrMatchPipelines = _.isArray(matchPipelines) ? matchPipelines : [matchPipelines];
                        _.forEach(arrMatchPipelines, (currPipeline) => {
                            pipelineIds.push(currPipeline.info.id);
                        });
                    } else {
                        printError('Cannot find any workflows with these pipelines names');
                        process.exit(0);
                    }
                });
            }
