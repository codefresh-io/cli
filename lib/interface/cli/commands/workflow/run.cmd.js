...
const run = new Command({
    ...
    handler: async (argv) => {
        ...
        _.forEach(executionRequests, async ({pipelineId, options}) => {
            const promise = ObjectID.isValid(pipelineId) ?
                pipeline.runPipelineById(pipelineId, options) :
                pipeline2.runPipelineByName(pipelineId, options);
            const workflowId = await promise;
