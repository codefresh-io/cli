    ...
    builder: (yargs) => {
        return yargs
            ...
            .option('force-prepare', {
                describe: `Refresh the token even if the current token is still valid. Unlike \`--prepare\`, which only
                 refreshes expired tokens, this flag forces a token refresh unconditionally`,
            })
