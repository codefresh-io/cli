...
const installAgentCmd = new Command({
    ...
    builder: yargs => yargs
        ...
        .option('skip-runtime-install', {
            describe: 'Do not install and attach runtime on the same namespace as the agent (default is false)',
        })
        .option('verbose', {
            describe: 'Print logs',
        }),
    handler: async (argv) => {
        ...
        let agent;
        let { 'skip-runtime-install': skipRuntimeInstall } = argv;
        if (skipRuntimeInstall === undefined || skipRuntimeInstall === null) {
        ...
        if (!token) { // Create an agent if not provided
            ...
            token = agent.token;
            console.log(`An agent with name: ${name} with token ${token} was created\n note this the last only time the token will be printted`);
        } else {
        ...
        await sdk.agents.install({
            ...
            verbose,
            agentId: name,
            terminateProcess: skipRuntimeInstall,
        });
