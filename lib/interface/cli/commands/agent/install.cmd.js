...
async function newRunnerName(kubeContextName, kubeNamespace) {
    const defaultName = `${kubeContextName}_${kubeNamespace}`;
    const runners = await sdk.agents.list({ });
    let name;

    if (!_.isArray(runners) || !_.find(runners, a => a.name === defaultName)) {
        name = defaultName; // use the default name if there are no collisions
    } else {
        const runnerNames = new Set(_.map(runners, r => r.name)); // for fast lookup
        let i = 1;
        while (runnerNames.has(`${defaultName}_${i}`)) {
            i += 1;
        }
...
const installAgentCmd = new Command({
    ...
    handler: async (argv) => {
        ...
        if (!token) { // Create an agent if not provided
            name = name || await newRunnerName(kubeContextName, kubeNamespace);
