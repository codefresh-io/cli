...
const create = new Command({
    ...
    handler: (argv) => {
        ...
        const { subCommands } = create.subCommands;
        let requiredSubCommand = null;
        for (let i = 0; i < subCommands.length; i++) {
            if (subCommands[i].rawCommand.webDocs.category.toLowerCase().includes(data.kind.toLowerCase())) {
                requiredSubCommand = subCommands[i];
                break;
            }
        }
        if (requiredSubCommand) {
            subCommands.getHandler()(argv);
        }
        else {
