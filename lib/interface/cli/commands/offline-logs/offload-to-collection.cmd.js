...
const { objectIdFromDate, defaultCollections, compareVersions, collectionExists } = require('./utils')
...
const offloadToCollection = async function(sourceDBObj, collection, targetDB, cutoffDate) {
  ...
  if (compareVersions(mergeReleaseVersion, mongoServerVersion) <= 0){
    await archiveWithMerge(sourceCollectionObj, cutoffDateId, targetDB, targetCollection, collection);
  } else {
    if (await collectionExists(sourceDBObj, targetCollection)) {
      await archiveInChunks(sourceDBObj, sourceCollectionObj, targetCollection, cutoffDateId);
    } else {
      await archiveWithOut(sourceCollectionObj, cutoffDateId, targetCollection, collection);
    }
  }
}

const archiveWithMerge = async function(sourceCollectionObj, cutoffDateId, targetDB, targetCollection, collection) {
...
const archiveWithOut = async function(sourceCollectionObj, cutoffDateId, targetCollection, collection) {
  var result = sourceCollectionObj.aggregate([
    { $match: { _id: { $lte: ObjectId(cutoffDateId) } } },
...
const archiveInChunks = async function(sourceDBObj, sourceCollectionObj, targetCollection, cutoffDateId, collection) {
  const targetCollectionObj = sourceDBObj.collection(targetCollection);
  const chunkSize = 1;
  const minLog = await sourceCollectionObj
...
const command = new Command({
    ...
    .example('codefresh offline-logs offload-to-collection --uri "mongodb://192.168.99.100:27017" --db logs --cod "2021-07-08" '),
    handler: async (argv) => {
      const {
        uri,
        db,
      ...
      try{
        ...
        if (failedCollections.length){
          throw new Error(`failed to offload from collections: ${failedCollections.join(', ')}. ${errors.join(', ')}`)
        }
      } finally {
          client.close();
      }
