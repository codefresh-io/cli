...
const compareVersions = function(oneVersion, otherVersion) {
  const oneVersionParts = oneVersion.split('.');
  const otherVersionParts = otherVersion.split('.');
  const expectedArrLength = 3;

  while (oneVersionParts.length < expectedArrLength) oneVersionParts.push("0");
  while (otherVersionParts.length < expectedArrLength) otherVersionParts.push("0");

  if (oneVersionParts[0] < otherVersionParts[0]) {
    return -1;
  } else if (oneVersionParts[0] > otherVersionParts[0]) {
    return 1;
  }

  if (oneVersionParts[1] < otherVersionParts[1]) {
    return -1;
  } else if (oneVersionParts[1] > otherVersionParts[1]) {
    return 1;
  }

  if (oneVersionParts[2] < otherVersionParts[2]) {
    return -1;
  } else if (oneVersionParts[2] > otherVersionParts[2]) {
    return 1;
  }
...
const collectionExists = async function(dbObject, targetCollection) {
  const collectionsList = await dbObject.listCollections();
