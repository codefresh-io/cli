...
async function newRuntimeName(clusterName, kubeNamespace) {
    const defaultName = `${clusterName}/${kubeNamespace}`;
...
const installRuntimeCmd = new Command({
    ...
    handler: async (argv) => {
        ...
        const runtimeName = await newRuntimeName(clusterName, kubeNamespace);
