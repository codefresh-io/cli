...
const installRuntimeCmd = new Command({
    ...
    handler: async (argv) => {
        ...
        const clusterEvents = new ProgressEvents();
        const clusterFormat = 'downloading cluster installer  [{bar}] {percentage}% | {value}/{total}';
        const clusterProgressBar = new cliProgress.SingleBar({ stopOnComplete: true, format: clusterFormat }, cliProgress.Presets.shades_classic);
        let clusterTotalSize;
        clusterEvents.onStart((size) => {
            clusterProgressBar.start(size, 0);
            clusterTotalSize = size;
        });
        clusterEvents.onProgress((progress) => {
            clusterProgressBar.update(progress);
