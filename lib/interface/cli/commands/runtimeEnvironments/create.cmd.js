    ...
    handler: async (argv) => {
        ...
        if (!local) {
            ...
            fs.writeFile(filePath, response, (err) => {
                ...
                const k8sScript = spawn('bash', [filePath, '--api-token', currentContext.token, '--api-host', currentContext.url, '--image-tag', 'auto_creation_cluster', '--namespace', namespace, '--context', context, cluster]);
                callToScript(k8sScript);
            });
        } else {
            filePath = './codefresh-k8s-configure.sh';
            const k8sScript = spawn('bash', [filePath, '--api-token', currentContext.token, '--api-host', currentContext.url, '--image-tag', 'auto_creation_cluster', '--namespace', namespace, '--local','--context', context, cluster]);
            callToScript(k8sScript);
        }
    },
});
