...
const command = new Command({
    ...
    aliases: ['repo'],
    parent: addRoot,
    description: 'Add a repository from git provider ("context") to codefresh',
    webDocs: {
        category: 'Repository',
    ...
    handler: async (argv) => {
        ...
        try {
            ...
            const existing = await repository.get(fullname, context).catch(() => null); // throws on not found
            if (existing) {
