...
`);
        ...
        const runtimesToKeep = _.filter(runtimes, re => !attachedRuntimes.includes(_.get(re, 'metadata.name')));
        const clustersToDelete = _.filter(clusters, c => !_.find(runtimesToKeep, re => _.get(re, 'runtimeScheduler.cluster.clusterProvider.selector') === c.selector));
