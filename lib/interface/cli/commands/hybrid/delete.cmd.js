...
async function getRelatedNamespaces(kubeContextName, runtimes) {
    return runtimes.reduce((acc, cur) => {
        const ns = _.get(cur, 'runtimeScheduler.cluster.namespace');
        const cn = _.get(cur, 'runtimeScheduler.cluster.clusterProvider.selector');
        if (cn === kubeContextName && ns) {
            acc.push(ns);
        }
        return acc;
    }, []);
...
async function getRelatedAgents(kubeContextName, kubeNamespace, runtimes) {
    ...
    _.forEach(runtimes, (r) => {
        ...
        const cn = _.get(r, 'runtimeScheduler.cluster.clusterProvider.selector');
        if (cn === kubeContextName && ns === kubeNamespace) {
...
const deleteCmd = new Command({
    ...
    handler: async (argv) => {
        ...
        if (!kubeNamespace) {
            const relatedNamespaces = await getRelatedNamespaces(kubeContextName, runtimes);
        ...
        const agents = await getRelatedAgents(kubeContextName, kubeNamespace, runtimes);
        if (!agents.length) {
