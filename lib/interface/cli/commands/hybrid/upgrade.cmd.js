...
const { handleError, getRelatedAgents } = require('./helper');
...
function getStrategy(currentVersion, targetVersion) {
    if (currentVersion < 1 ) { // TODO:Semver 
        return migrateToOne;
    } else {
        return upgrade;
    }
}
async function migrateToOne({kubeContext, kubeNamespace, agentName}) {
...
const upgradeCmd = new Command({
    ...
    handler: async (argv) => {
        ...
        await handleError(listReErr, 'Failed to get runtime environments', openIssueMessage);
        const agents = await getRelatedAgents(kubeNamespace, runtimes, openIssueMessage);
