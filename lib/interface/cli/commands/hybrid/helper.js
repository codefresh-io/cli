...
async function createTestPipeline(runtimeName, pipelineName, pipelineCommands, containerRegistry) {
    ...
    pipeline.spec.steps.test = {
        stage: 'test',
        title: 'test',
        image: containerRegistry ? `${containerRegistry}/codefresh/alpine:3.11` : 'alpine:latest', // 1) works only for current default cr: quay. 2) the right side of the ternary will never occur. 3) if flag will be stated, image will fail to be pulled. 4) works only with the :3.11 tag, otherwise no manifest is found.
