...
function resolveDemoStepImage(dockerRegistry) {
    let demoStepImage = DEMO_STEP_IMAGE[dockerRegistry];
    if (!demoStepImage) {
        demoStepImage = DEMO_STEP_IMAGE.default;
    }

    return demoStepImage;
}
...
async function createTestPipeline(runtimeName, pipelineName, pipelineCommands, dockerRegistry) {
    ...
    let demoStepImage = resolveDemoStepImage(dockerRegistry);
