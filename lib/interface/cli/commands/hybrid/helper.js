...
async function _downloadVeonona() {
    ...
    if (error != null) {
        const newLocation = path.join(process.cwd(), components.venona.local.binary);
        if (await pathExists(newLocation)) {
            console.log('Failed to download insatller, using local version');
            return INSTALLATION_DEFAULTS.COMPONENETS_INSTALLATION_PATH;
        }
        console.log('Failed to download component, aborting');
        throw error;
    }
    return downloader.location;
}
async function _downloadSteveDore() {
    const downloader = new Downloader({
        progress: new cliProgress.SingleBar(
            {
                stopOnComplete: true,
    ...
    await downloader.download(components.stevedore);
}

async function downloadHybridComponents() {
    await _downloadVeonona();
    console.log('Kubernetes components installer downloaded successfully');
    await _downloadSteveDore();
    console.log('Kubernetes registrator installer downloaded successfully');
}
