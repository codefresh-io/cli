...
async function attemptDownload(location, component) {
    console.log('do you mean THIS Downloading installer');
    console.log('RIGHT AFTER ENTERING ATTEMPT DOWNLOAD');
    ...
    console.log('RIGHT BEFORE SENDING TO DOWNLOAD');
    return await to(downloader.download(component));
}

async function downloadVeonona(location = CODEFRESH_PATH, bypassDownload) { 
    ...
    console.log('Failed to use binary from components folder.');
...
async function downloadProvider({ provider, location = CODEFRESH_PATH }, bypassDownload) {
...
async function downloadSteveDore(location = CODEFRESH_PATH, bypassDownload) {
...
async function downloadHybridComponents(location, bypassDownload) { // TODO: make sure it actually gets the bypassDownload here
