...
async function _downloadVeonona(location) {
    ...
    if (error) {
        ...
        console.log('Failed to download installer, aborting');
        throw error;
    }
    return downloader.location;
}
async function _downloadSteveDore(location) {
    const downloader = new Downloader({
        progress: new cliProgress.SingleBar(
            {
                stopOnComplete: true,
...
async function downloadHybridComponents(location) {
    await _downloadVeonona(location);
    await _downloadSteveDore(location);
}

async function runClusterAcceptanceTests({ kubeNamespace, kubeConfigPath }) {
    const location = await _downloadVeonona();
    const componentRunner = new Runner(location);
