...
const initCmd = new Command({
    ...
    handler: async (argv) => {
        ...
        const proxyEnvVars = detectProxy();
        let httpProxy;
        let httpsProxy;
        if (noQuestions) {
            // use defaults
            ...
            questions.push({
                type: 'input',
                name: 'httpProxy',
                default: proxyEnvVars.httpProxy,
                message: 'HTTP proxy to be used by runner inside Kubernetes (default is none)  ?',

            });
            questions.push({
                type: 'input',
                name: 'httpsProxy',
                default: proxyEnvVars.httpsProxy,
                message: 'HTTPS proxy to be used by runner inside Kubernetes (default is none) ?',
        ...
        let envVars;
        if (httpProxy) {
            envVars = { http_proxy: httpProxy, HTTP_PROXY: httpProxy };
        }
        if (httpsProxy) {
