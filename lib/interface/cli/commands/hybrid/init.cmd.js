...
async function createDemoPipeline(runtimeName, progress) {
    ...
    await to(installationProgress.report(sdk['runner-installation'], progress.id, {
        event: installationProgress.events.PIPELINE_CREATED,
        content: {
            status: 'success',
        },
    }));
...
async function createAndExecuteDemoPipeline(runtimeName, progress) {
    ...
    if (!demoPipelineExists) {
        ...
        const [createDemoPipelineError] = await to(createDemoPipeline(runtimeName, progress));
        handleError(createDemoPipelineError, 'Failed to create demo pipeline');
    } else {
    ...
    handleError(pipelineExecutionError, 'Failed to run demo pipeline');

    await to(installationProgress.report(sdk['runner-installation'], progress.id, {
        event: installationProgress.events.PIPELINE_EXECUTED,
        content: {
            status: 'success',
        },
    }));
...
`);
        ...
        if (token) { // Add context
            ...
            handleError(err, 'Failed to use the provided token');
        ...
        handleError(err, 'Runner installation failed');


        await to(installationProgress.report(sdk['runner-installation'], progress.id, {
            event: installationProgress.events.RUNNER_INSTALLED,
            content: {
                status: 'success',
            },
        }));
        ...
        await to(installationProgress.report(sdk['runner-installation'], progress.id, {
            event: installationProgress.events.MONITOR_INSTALLED,
            content: {
                status: 'success',
            },
        }));
        ...
        if (shouldExecutePipeline) {
            await createAndExecuteDemoPipeline(runtimeName, progress);
        }
        ...
        await to(installationProgress.report(sdk['runner-installation'], progress.id, {
            event: installationProgress.events.FINISHED,
            content: {
                status: 'success',
            },
        }));
