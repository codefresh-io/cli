...
const defaultContainerRegistry = 'quay.io';
...
const initCmd = new Command({
    ...
    builder: yargs => yargs
        ...
        .option('container-registry', {
            describe: `The prefix for the container registry that will be used for pulling the required components images. Example: --container-registry="docker.io"  default is ${defaultContainerRegistry}`,
            type: 'string',
            default: defaultContainerRegistry,
        })
    ...
    handler: async (argv) => {
        ...
        // update runtime images with container-registry
        installationPlan.addStep({
            name: 'update runtime environment images with container-registry url',
            func: async () => {
                const reName = installationPlan.getContext('runtimeName');
                const re = await sdk.runtimeEnvs.get({ name: reName });
                const body = _.set({}, 'runtimeScheduler.envVars', getRuntimeImagesWithRegistryUrl(containerRegistry));
                _.set(body, 'runtimeScheduler.image', `${containerRegistry}/codefresh/engine:master`);
                _.set(body, 'dockerDaemonScheduler.dindImage', `${containerRegistry}/codefresh/dind:18.09.5-v24-sp`);
                await sdk.runtimeEnvs.update({ name: reName }, _.merge(re, body));
                console.log(`Runtime environment "${colors.cyan(reName)}" has been updated with the specified container-registry images`);
            },
            condition: !!containerRegistry && containerRegistry !== defaultContainerRegistry,
        });
        ...
        if (shouldExecutePipeline) {
            ...
            if (!testPipelineExists) {
                ...
                installationPlan.addStep({ 
                    ...
                    func: async () => {
                        await updateTestPipelineRuntime( // TODO: should consider this as well.
                            undefined,
                            installationPlan.getContext('runtimeName'),
                            INSTALLATION_DEFAULTS.DEMO_PIPELINE_NAME,
                        );
