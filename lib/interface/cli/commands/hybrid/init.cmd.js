...
async function createAndExecuteDemoPipeline(runtimeName, progressReporter) {
    ...
    if (!demoPipelineExists) {
        ...
        const [createDemoPipelineError, pipeline] = await to(createDemoPipeline(runtimeName, progressReporter));
        ...
        console.log(`Created demo pipeline with the name "${colors.cyan(pipeline.metadata.name)}". Watch it here: ${colors.blue(pipelineLink)}`);
    } else {
        const pipeline = pipelines.docs[0];
        const pipelineLink = `${url}/pipelines/edit/workflow?id=${pipeline.metadata.id}&pipeline=${encodeURI(pipeline.metadata.name)}`;
        pipelineName = pipeline.metadata.name;
        console.log(`Demo pipeline with the name: "${colors.cyan(pipeline.metadata.name)}" already exists. Watch it here: ${colors.blue(pipelineLink)}`);
    }
