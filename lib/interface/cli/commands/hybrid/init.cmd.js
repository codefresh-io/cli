...
const initCmd = new Command({
    ...
    handler: async (argv) => {
        ...
        questions.push({
            ...
            default: true,
            message: 'Should all pipelines run on the hybrid runtime ?',

        });

        console.log(colors.green('This installer will guide you through the hybrid installaton prorcess'));
        const answers = await inquirer.prompt(questions);
        kubeContextName = kubeContextName || answers.context;
        kubeNamespace = kubeNamespace || answers.namespace;
        const { shouldMakeDefaultRe } = answers;
        ...
        console.log(colors.green('Documenation link : https://codefresh.io/docs/docs/enterprise/codefresh-runner/#codefresh-runner-preview-release'));
    },
});
