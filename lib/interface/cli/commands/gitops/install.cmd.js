...
const installArgoCmd = new Command({
    ...
    builder: yargs => yargs
        ...
        .option('load-balancer', {
            describe: 'By default, the Argo CD API server is not exposed with an external IP. To access the API server change the argocd-server service type to LoadBalancer',
            type: 'boolean',
            default: false,
        }),
    handler: async (argv) => {
        ...
        if (provider === 'argocd') {
            // install argocd
            await installArgocd({ installManifest, kubeNamespace, loadBalancer });
