...
const command = new Command({
    ...
    builder: (yargs) => {
        return yargs
            .option('exec-path', {
                alias: 'p',
                description: 'Path to your codefresh executable (at most cases defined automatically)',
            })
            .option('alias', {
                alias: 'a',
            ...
            .example('codefresh completion --exec-path /some/full/path/codefresh', 'Print completion script with specified path to codefresh executable');
    },
    handler: async (argv) => {
        const { 'exec-path': executablePath, alias: appName } = argv;
        const appPath = executablePath || yargs.$0; // "yargs.$0" -- parsed from process.argv (default)
