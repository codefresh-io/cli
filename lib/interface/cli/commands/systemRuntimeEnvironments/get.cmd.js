...
const { serializeObjectAsQueryParam } = require('../../helpers/general');
...
const command = new Command({
    ...
    handler: async (argv) => {
        ...
        if (history) {
            const allHistory = await sdk.system.re.get({
                name,
                history,
                plan,
            });
            ...
            const allSuccessors = await sdk.system.re.get({
                name,
                successors,
                plan,
            });
            ...
            const currRuntimeEnvironments = await sdk.system.re.get({
                name,
                version,
                extend,
                plan,
            ...
            let re = await sdk.system.re.getAll({
                limit,
                offset,
                account,
                accountIds: serializeObjectAsQueryParam(accountIds, 'accountIds'),
            });
            if (!_.isEmpty(accountIds)) {
                re = _.flatten(re.map(({ runtimeEnvironments }) => runtimeEnvironments));
            }
            Output.print(re.map(RuntimeEnvironments.fromResponse));
        }
    },
});
