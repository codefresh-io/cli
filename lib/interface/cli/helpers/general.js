...
const prepareKeyValueObjectsFromEnvFileOption = (environmentVariables) => _.map(environmentVariables, (value, key) => {
    if (_.isObject(value)) {
        const { val, encrypted } = value;
        return { key, value: val, encrypted };
    }
    return { key, value };
});

const prepareEncryptedValues = (variables, encrypted) => {
        ...
        .coerce(options.name || 'filename', (arg) => {
            try {
                ...
                throw new CFError('Not a valid file.\n'
                    + ' for more info how to pass valid file doc link: https://codefresh-io.github.io/cli/pipelines/run-pipeline/');
            } catch (err) {
