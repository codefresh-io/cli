...
const crudFilenameOption = (yargs, options = {}) => {
    const filenameOption = {
        ...
        describe: options.describe || 'Filename to use to create the resource',
    };
    if (options.group) {
...
const callToSubCommandHandler = (argv , command) => {
    const data = argv.filename;
    const subCommands = command.subCommands;
    const requiredSubCommand = _.find(subCommands, ((subCommand) => {
        if (subCommand.rawCommand.webDocs.category.toLowerCase().includes(data.kind.toLowerCase())) {
            return subCommand;
        }
    }));
    if (requiredSubCommand) {
        requiredSubCommand.getHandler()(argv);
    } else {
        throw new CFError('Illegal type of resource');
    }
};
