...
async function getApiVersion(token) {
    const RequestOptions = {
        ...
        retryStrategy: request.RetryStrategies.HTTPOrNetworkError, // Retry on HTTP or network errors
    };
    try {
...
const wrapHandler = (handler, requiresAuthentication) => {
    return async (argv) => {
        try {
            ...
            const apiVersion = token ? await getApiVersion(token) : null;
            _.set(argv, `${defaults.CLI_INTERNAL_CONFIG}.apiVersion`, apiVersion);
            if (!argv.watch) {
                await handler(argv);
                return;
            }
