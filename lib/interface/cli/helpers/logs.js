...
const followLogs = async (workflowId) => {
    try {
        checkVersion(engines.node);
    } catch (error) {
        const url = _.get(sdk, 'config.context.url', 'https://g.codefresh.io');
        const buildLink = `${url}/build/${workflowId}`;
        // eslint-disable-next-line max-len
        console.log(`Your node version (${process.version}) is not compatible with supported node version (${engines.node}) for this command, click the link to see it in codefresh UI : ${buildLink}`);
        return;
    }
    try {
        await sdk.logs.showWorkflowLogs(workflowId, true);
