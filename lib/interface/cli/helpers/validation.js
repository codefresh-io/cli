...
async function validatePipelineSpec(data) {
    ...
    const result = await sdk.pipelines.validateYaml({}, { requestBody: { yaml: validatedYaml } });
    let message;
    if (!result.valid) {
...
async function validatePipelineYaml(filename, fileContent) {
    ...
    const result = await sdk.pipelines.validateYaml({}, { requestBody: { yaml: yamlContent } });
    let message;
    if (!result.valid) {
