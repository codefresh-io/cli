...
async function validatePipelineSpec(data) {
    const yamlObj = Object.assign({ version: data.version });
    if (_.get(data, 'spec.steps')) {
        yamlObj.steps = _.get(data, 'spec.steps');
    }
    if (_.get(data, 'spec.stages')) {
        yamlObj.stages = _.get(data, 'spec.stages');
    }
