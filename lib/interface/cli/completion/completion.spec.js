...
describe('codefresh completions', () => {
    ...
    describe('filedir', () => {
        ...
        it('should list dirs and yaml files when no prefix passed', async () => {
            process.argv = ['codefresh', 'get', '--impl-file-dir', '-f', ''];
            const result = await getCompletion(process.argv);
            expect(result).toEqual(['__files_completion__', 'libe/', 'like/', 'another/']);
        });

        it('should display next commands completions when option is provided with a value', async () => {
            process.argv = ['codefresh', 'get', '--impl-file-dir', '-f', 'li'];
            const result = await getCompletion(process.argv);
            expect(result).toEqual(['__files_completion__', 'libe/', 'like/']);
        });

        it('should display next commands completions when option is provided with a value', async () => {
            process.argv = ['codefresh', 'get', '--impl-file-dir', '-f', 'lib'];
            const result = await getCompletion(process.argv);
            expect(result).toEqual(['__files_completion__', 'libe/1', 'libe/2']);
        });
        it('should display next commands completions when option is provided with a value', async () => {
            process.argv = ['codefresh', 'get', '--impl-file-dir', '-f', 'another'];
            const result = await getCompletion(process.argv);
        ...
        it('should display next commands completions when option is provided with a value', async () => {
            process.argv = ['codefresh', 'get', '--impl-file-dir', '-f', 'lib/cli'];
            const result = await getCompletion(process.argv);
            console.log(result);
        });
    });
